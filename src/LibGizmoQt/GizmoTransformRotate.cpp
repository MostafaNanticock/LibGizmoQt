...
bool CGizmoTransformRotate::CheckRotatePlan(tvector3 &vNorm, float factor, const tvector3 &rayOrig, const tvector3 &rayDir, int id)
{
	tvector3 df, inters;
	m_Axis2 = vNorm;
...
bool CGizmoTransformRotate::GetOpType(ROTATETYPE &type, unsigned int x, unsigned int y)
{
	...
	if (mMask & AXIS_SCREEN)
		if (CheckRotatePlan(dir, 1.2f, rayOrigin, rayDir, -1))
		{
			...
			m_Axis *= tvector3(GetTransformedVector(0).Length(), GetTransformedVector(1).Length(), GetTransformedVector(2).Length());
...
inline void CGizmoTransformRotate::SetSnap(const float snap)
{
	m_AngleSnap = snap;
}

inline void CGizmoTransformRotate::SetSnap(float snapx, float snapy, float snapz)
{}

inline float CGizmoTransformRotate::GetAngleSnap()
{
	return m_AngleSnap;
}
